...
class RelevanceRepository:
    ...
    async def get(self, feedback_id: str, *, user_id: str) -> RelevanceRecord | None:
        ...
        rows = await self._query(
            ...
            "AND op_type = 'RELEVANCE_RECORDED'", relevance_operation_id(user_id, feedback_id), user_id,
        )
        return self._read_record(rows[0]["payload"], user_id=user_id, feedback_id=feedback_id) if rows else None
