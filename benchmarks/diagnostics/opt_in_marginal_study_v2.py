...
def register():
    ...
    reg = {'kind': 'label-free-marginal-packing-source-assay', 'registered_at': study.utc(),
        ...
        'scheduling': 'Amendment v2 starts five local source tasks after authenticating the complete baseline. No provider calls during source assay. If advanced, freeze all answer contexts, then wait for every fixed historical retrieval arm to finalize and authenticate before running control then candidate reader stages, each four provider slots under disclosed shared load. Fixed matrix capacity and policies are unchanged.',
        'unexecuted_predecessor': 'opt-in-marginal-packing-v1: registered but withdrawn before any source case or answer call; only its idle waiting process was stopped. Scheduling-only change; all policies, selection gates, contexts and scoring are unchanged.',
...
def answer_followup(reg, rows, selected):
    ...
    for kind, source in (('control', 'control'), ('candidate', selected)):
        ...
        names[kind] = name
    parent = json.loads((study.REPORTS / 'opt-in-successor-v2-registration.json').read_text())
    while not all(settled(study.PRIVATE / 'opt-in-successor-v2' / a['id'])
