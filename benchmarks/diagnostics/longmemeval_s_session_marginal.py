"""Evaluate session-marginal context packing on frozen LongMemEval-S packs.

This full-development source-retention assay compares the current product
packer with a preregistered grid that applies diminishing marginal utility to
repeated ordinary records from the same exact session. It does not call a reader
or judge and cannot promote a configuration without a later answer-quality trial
and untouched confirmation.
"""
...
WORKERS = 5
ARM_CONFIGS: dict[str, dict[str, int | float | None]] = {
    "control": {"free_slots": None, "decay": None},
    "marginal_f1_d098": {"free_slots": 1, "decay": 0.98},
    "marginal_f2_d098": {"free_slots": 2, "decay": 0.98},
    "marginal_f4_d098": {"free_slots": 4, "decay": 0.98},
    "marginal_f2_d095": {"free_slots": 2, "decay": 0.95},
    "marginal_f4_d095": {"free_slots": 4, "decay": 0.95},
    "marginal_f4_d090": {"free_slots": 4, "decay": 0.90},
    "marginal_f8_d090": {"free_slots": 8, "decay": 0.90},
}
...
def _protocol() -> dict[str, Any]:
    return {
        ...
        "session_boundary": "exact (scope, session_id) among authorized candidates",
        "marginal_utility": (
            "within ordinary evidence tiers, the first free_slots successfully "
            "packed records retain their existing value; each later record from "
            "that session multiplies value by decay once more"
        ),
        "preserved_priorities": (
            "required records, instructions, pins, active tasks and explicitly "
            "routed episode/evidence records retain their existing order"
        ),
        "invariants": [
            ...
            "the packer sees no references, evidence labels, answers or question types",
...
def create_registration(
    *,
    ...
    registration = {
        "schema_version": 1,
        "kind": "longmemeval-s-session-marginal-registration",
        "registered_at": datetime.now(timezone.utc).isoformat(),
        "status": "registered_before_session_marginal_grid_evaluation",
        "claim_boundary": (
            "Full-development source-retention assay for experimental deterministic "
            "session-marginal packing; not answer accuracy, an independent holdout, "
            "a multi-scope representation test, or a Zep comparison."
        ),
        "source": {
...
def _validate_registration(
    ...
    *,
    ...
    if (
        ...
        or registration.get("kind") != "longmemeval-s-session-marginal-registration"
        or registration.get("status")
        != "registered_before_session_marginal_grid_evaluation"
        ...
        raise ValueError("registered session-marginal inputs differ")
    by_id = {case["question_id"]: case for case in cases}
    return [
        by_id[question_id] for question_id in registration["dataset"]["question_ids"]
    ]
    ...
    *,
    ...
    if output_path.exists():
        ...
        if saved.get("registration_sha256") != registration_sha256 or saved.get(
            ...
            raise ValueError(f"saved session-marginal case differs for {question_id}")
        return saved

    baseline = json.loads(
        (baseline_root / "captures" / f"{question_id}.json").read_text()
    )
    with tempfile.TemporaryDirectory(prefix="prme-lme-session-marginal-") as temporary:
        ...
        try:
            ...
            for name, updates in ARM_CONFIGS.items():
                ...
                if free_slots is None:
                    ...
                    if not isinstance(free_slots, int) or not isinstance(decay, float):
                        raise ValueError(f"invalid candidate arm {name}")
                    bundle = pack_context(
                        response.results,
                        config.packing,
                        coverage_notice=response.bundle.coverage_notice,
                        context_guidance=guidance,
                        _session_marginal_free_slots=free_slots,
                        _session_marginal_decay=decay,
                    )
                ...
                session_counts = Counter(
                    (candidate.node.scope.value, candidate.node.session_id)
                ...
                arms[name] = {
                    ...
                    "session_records": sum(session_counts.values()),
                    "unique_sessions": len(session_counts),
                    "max_session_records": max(session_counts.values(), default=0),
    ...
    result = {
        "schema_version": 1,
        "kind": "longmemeval-s-session-marginal-case",
...
def _arm_summary(rows: list[dict[str, Any]], arm: str) -> dict[str, Any]:
    ...
    unique_sessions = [int(row["arms"][arm]["unique_sessions"]) for row in rows]
    max_session_records = [int(row["arms"][arm]["max_session_records"]) for row in rows]
    return {
        "questions": len(rows),
        ...
        "session_records": sum(row["arms"][arm]["session_records"] for row in rows),
        "unique_sessions": {
            "mean": statistics.mean(unique_sessions),
            "median": statistics.median(unique_sessions),
            "max": max(unique_sessions),
        },
        "max_session_records": {
            "mean": statistics.mean(max_session_records),
            "median": statistics.median(max_session_records),
            "p95": _percentile([float(value) for value in max_session_records], 0.95),
            "max": max(max_session_records),
        },
    }


def _comparison(rows: list[dict[str, Any]], arm: str) -> dict[str, Any]:
    applicable = [
...
def _candidate_gate(
    *,
    ...
    gate = {
        "budget_violations": sum(row["arms"][arm]["tokens"] > 3996 for row in rows),
...
def _summary(rows: list[dict[str, Any]], registration_path: Path) -> dict[str, Any]:
    ...
    result = {
        "schema_version": 1,
        "kind": "longmemeval-s-session-marginal-result",
        ...
        "decision": ("advance_to_answer_development" if selected else "reject_grid"),
        "runtime": {
    ...
    *,
    ...
    async def run_one(case: dict[str, Any]) -> dict[str, Any]:
        ...
        if completed % 10 == 0 or completed == len(cases):
            print(
                f"[longmemeval-s-session-marginal] completed {completed}/{len(cases)}",
                flush=True,
            )
        return value
    ...
    if errors:
        raise RuntimeError(
            f"session-marginal packing failed for {len(errors)} case(s)"
        ) from errors[0]
...
def main() -> None:
    ...
    if args.command == "register":
        registration = create_registration(output_path=args.output.resolve(), **shared)
        print(
            json.dumps(
                {
