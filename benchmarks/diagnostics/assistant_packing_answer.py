"""Evaluate density versus balanced packing on the known assistant-memory gap.

This is a focused development diagnostic. Registration freezes the existing
verified contexts and model identities before generation. The reader receives
questions and contexts without references; a separately calibrated model judges
complete predictions afterward. A failed provider call is retained and cannot
be replaced by resuming the same study.
"""
...
READER_MODEL = "prme-qwen3.5:35b-a3b-8k"
...
def source_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        "composition": data / "packing-composition-dev-273fd96.json",
        "composition_completion": evidence / "packing-composition-dev-completion-273fd96.json",
        "composition_verification": evidence / "packing-composition-dev-verification.json",
        "verifier_completion": evidence / "packing-composition-dev-verifier-completion.json",
        "inputs": data / "hindsight-prme-dev-normalized-inputs.json",
        "references": data / "hindsight-prme-dev-references.json",
        "snapshots": data / "hybrid-lexical-dev-a744fe0-snapshots",
        ...
        "prior_reader_registration": evidence / "hindsight-prme-reader-registration.json",
    }


def _source_identity(paths: dict[str, Path]) -> dict[str, str]:
    return {
...
def prepare(root: Path) -> dict:
    """Produce neutral reader inputs and verify current balanced-policy parity."""
    ...
    for result in composition["details"]:
        ...
        snapshot_path = paths["snapshots"] / snapshot_ref["filename"]
    ...
    if len(rows) != 9 or len({row["case_id"] for row in rows}) != len(rows):
        raise ValueError("Expected exactly nine assistant-memory development questions")
    return {
        "kind": "assistant-packing-answer-inputs",
        "schema_version": 1,
        "budget": BUDGET,
...
def reader_declaration(base_url: str) -> dict:
    return {
        "model": READER_MODEL,
        "model_digest": runtime.model_digest(base_url, READER_MODEL),
...
def _request(base_url: str, body: dict) -> dict:
...
def reader_payload(row: dict, arm: str, declaration: dict) -> dict:
    ...
    if sum(len(message["content"].encode()) for message in messages) > 28000:
...
def _answer(response: dict, model: str) -> str:
    ...
    if (
        ...
        raise ValueError("Reader response is incomplete or invalid")
    if any(type(response.get(field)) is not int for field in ("prompt_eval_count", "eval_count")):
        raise ValueError("Reader response is missing token observations")
    if response["prompt_eval_count"] + response["eval_count"] > READER_OPTIONS["num_ctx"]:
        raise ValueError("Reader response exceeds the requested context")
    return answer.strip()
...
def register(root: Path, directory: Path, registration: Path, base_url: str) -> None:
    ...
    reader_judge.validate_calibration(controls, calibration, judge)
    ...
    write(registration, {
        "kind": "assistant-packing-answer-development",
        ...
        "limits": [
            "Focused post-hoc diagnosis of nine previously examined development questions.",
            "One local reader and a custom calibrated local judge; no independent holdout.",
            "Source support and judged answer correctness are separate measurements.",
            "Complete coverage and zero provider failures are required; failed calls cannot be retried in place.",
            "No default-policy or competitive leadership claim follows from this diagnostic.",
        ],
    })
...
def verify_registration(root: Path, directory: Path, registration: Path,
    ...
    if (
        ...
        or declared["prepared_sha256"] != digest((directory / "prepared.json").read_bytes())
        or declared["source_identity"] != _source_identity(paths)
...
def run_reader(prepared: dict, declared: dict, state_path: Path,
    ...
    for row in prepared["rows"]:
        for arm in sorted(ARMS, key=lambda value: digest(canonical([row["case_id"], value]))):
    ...
    with runtime.exclusive_state(state_path):
        ...
        for saved in state["generations"].values():
            ...
            _answer(saved["response"], declared["model"])
        write(state_path, state)
        for index, (_, _, body, key) in enumerate(jobs):
            if key not in state["generations"]:
                response = None
                try:
                    if runtime.model_digest(base_url, declared["model"]) != declared["model_digest"]:
                        raise ValueError("Reader model changed before generation")
                    response = _request(base_url, body)
                    _answer(response, declared["model"])
                    if runtime.model_digest(base_url, declared["model"]) != declared["model_digest"]:
    ...
    return {
        ...
        "rows": [{
            ...
            "answer": _answer(state["generations"][key]["response"], declared["model"]),
        } for row, arm, _, key in jobs],
    }


def _score(reader: dict, judge: dict, mapping: list[dict], limits: list[str]) -> dict:
    verdicts = {row["id"]: row["correct"] for row in judge["judgments"]}
    outcomes: dict[str, dict[str, bool]] = {}
    for row in mapping:
        outcomes.setdefault(row["case_id"], {})[row["arm"]] = verdicts[row["id"]]
    details = [{"case_id": case_id, "answers": values}
               for case_id, values in sorted(outcomes.items())]
    totals = {arm: sum(row["answers"][arm] for row in details) for arm in ARMS}
    paired = {}
    for before, after in (("empty", "density"), ("empty", "balanced"), ("density", "balanced")):
        pairs = [(row["answers"][before], row["answers"][after]) for row in details]
...
def run(root: Path, directory: Path, registration_path: Path, base_url: str) -> dict:
    ...
    references = json.loads(source_paths(root)["references"].read_bytes())["references"]
    refs = {row["case_id"]: row for row in references}
    questions = {row["case_id"]: row["question"] for row in prepared["rows"]}
    judge_cases = []
    mapping = []
    for row in reader["rows"]:
        identity = digest(canonical([row["case_id"], row["arm"]]))
        reference = refs[row["case_id"]]
        judge_cases.append({
            "id": identity,
    ...
    result = _score(reader, judge, mapping, registration["limits"])
    result.update(
...
def main() -> None:
    ...
    if args.mode == "register":
        ...
        print(json.dumps({key: result[key] for key in ("complete", "correct", "paired")}))
