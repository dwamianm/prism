...
def register():
    ...
    reg = {'kind': 'full-cohort-reranker-score-scale-repair', 'registered_at': study.utc(),
        ...
        'scheduling': 'Wait for all eleven historical fixed arms to settle and authenticate successes. Five source tasks; no hosted provider calls during source assay. Reader stages then use four slots each sequentially, sharing load with other disclosed experiments.',
...
async def run(args):
    ...
    while not all(settled(CONTROL_ROOT / a['id']) for a in parent['arms'] if a['stratum'] == 'historical'):
        if not args.wait:
            raise study.old.ResearchFailure('Historical stage not finalized')
