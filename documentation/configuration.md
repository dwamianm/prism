...
| `PRME_RERANKER_PRIOR_WEIGHT` | `0.3` | Weight of a reranked candidate's own score in the new order; the cross-encoder gets the rest. `0.0` orders the reranked candidates by the cross-encoder alone (issue #88). A value other than 0.3 is recorded in retrieval receipts and makes a learned ranking profile built under another value inapplicable |
