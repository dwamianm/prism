...
| Claim qualifiers | Built-in extraction preserves typed polarity, exact explicit conditions, and literal first-person attempts or intentions; it excludes unresolved conditions from default retrieval. Explicit condition evaluation is atomic, evidence-aware, idempotent across restarts, and available on Python, HTTP, and MCP; confirmed conditions receive asserted retrieval weight. A DeepSeek development probe on the exact BEAM ESLint source changed the false `uses_style_guide` claim to `trying_to_set_up`. | Authored local probes and one inspected BEAM development source, not held-out accuracy. V8 passed 11/14 speech-act cases; v9 eliminated unsafe claims and passed 13/14; v10 repeated the missing CUDA target. Corrected v11 retained CUDA but the hosted model omitted a Phoenix hope claim. V12 generalizes narrow exact-source/entity recovery across nonactual cues and requires confirmation. The validator is not general entailment. |
