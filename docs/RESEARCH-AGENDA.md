...
| Storage and developer workflow | [Atomic organizer merge validation](../benchmarks/results/recovery/2026-09-12/ATOMIC-ORGANIZER-MERGES.md): full regression at `31946c2`, 2,793 passed and 81 skipped including live PostgreSQL, research and examples. Final `4e43d0b` serialization guard passed 65 source and 183 installed Python 3.13 checks plus a real-BGE workflow. | Separate overlapping invocations; the full suite precedes the final serialization guard. Authored recovery contracts do not establish answer quality or all deployment environments. |
...
|---|---|
| Named projects and domains | Enforce an explicit namespace through reads, writes, graph/index candidates, derivation, maintenance, receipts and migration on both backends. The [current design review](NAMESPACE-DESIGN-REVIEW.md) compares physical and shared-table partitions against observed code constraints and current primary sources. A metadata filter alone cannot prevent cross-project merges. The [completed local resource probe](../benchmarks/results/recovery/2026-09-12/PARTITION-RESOURCES.md) favors a bounded active-engine prototype over keeping every pack open. Explicit DuckDB worker limits reduce thread growth, while memory still grows with resident packs. Separate packs remain the supported workaround. PostgreSQL partition/pool costs, full grants and hierarchy require their own tests. |
