...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. The first [external WiCE oracle-retrieval assay](../benchmarks/results/research/2026-09-16/WICE-CLAIM-VERIFICATION-V1.md) evaluated all 358 human-annotated test claims. It accepted 0/32 fully unsupported claims but passage-wide guards reduced supported recall to 10.91%. Schema 3 now localizes speech-act and negation guards to retained sentence-numbered evidence. The [registered causal confirmation](../benchmarks/results/research/2026-09-16/WICE-CLAIM-VERIFICATION-LOCALIZED-V2.md) preserved all eight authored gates and improved WiCE recall to 30.91%, F1 to 43.59% and balanced accuracy to 63.04%, while still accepting 0/32 fully unsupported claims. It nevertheless failed unchanged precision, recall and balanced-accuracy gates. | WiCE uses perfect evidence retrieval, so this is not PRME retrieval or original full-document performance. V2 reused the observed cohort and shows a causal passage-scoping improvement, not untouched generalization. The generic pinned NLI model's raw recall is only 41.82% and it confuses partial support. Keep verification opt-in; test a purpose-trained permissively licensed grounding model, then require another external cohort and model family before promotion. |
