...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Pinned MiniCheck trials culminated in a preregistered [atomic follow-up](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-FLAN-ATOMIC-V1.md): human decomposition reconstructed 357/358 parents, while the model reached 76.04% precision and 66.36% recall. A fresh ten-domain [SummEdits proof trial](../benchmarks/results/research/2026-09-16/SUMMEDITS-PROOF-VERIFICATION-V1.md) rejected free-form model quotations. The causal [separated segment-ID v2](../benchmarks/results/research/2026-09-16/SUMMEDITS-SEGMENT-VERIFICATION-V2.md) raised reference integrity from 23.00% to 99.00%, recall from 6.00% to 72.00%, and balanced accuracy from 52.00% to 76.00%, but still failed safety at 78.26% precision and 20.00% false support. | WiCE assumes perfect retrieval and SummEdits uses controlled edits over 40 repeated source families. Keep verification opt-in. Test an independent critic over fixed v2 atoms; require an untouched cohort before any provider verifier becomes product surface. |
