...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Causal guards and caller-order preservation passed same-cohort confirmations. A new [44-case previously unexecuted guard assay](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-GUARD-GENERALIZATION-V1.md) retained all ten completed-action refusals, 6/6 explicit refutations, 6/6 new chains, 4/4 conflicts and 4/4 completeness boundaries but failed on four wrong-relation negatives. Exact proposition alignment then passed all eight gates in a [same-cohort confirmation](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-GUARD-ALIGNMENT-V2.md): 43/44, zero unsafe supports, zero false refutations, 16/16 insufficient cases and identical raw scores. | Every passing fix confirmation reused an observed cohort. The newest cohort was untouched only before its failed v1; v2 is causal regression evidence. One typed-unverified paraphrase remains neutral, implicit contradictions require explicit or typed exclusivity, and the model remains order-sensitive. These are authored probes with one pinned model. Keep verification opt-in pending an external or separately authored confirmation and long-context evidence. |
