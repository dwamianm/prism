...
| Claim qualifiers | Built-in extraction preserves typed polarity, exact explicit conditions, and literal first-person attempts or intentions; it excludes unresolved conditions from default retrieval. Explicit condition evaluation is atomic, evidence-aware, idempotent across restarts, and available on Python, HTTP, and MCP; confirmed conditions receive asserted retrieval weight. The preregistered [v12 speech-act development confirmation](../benchmarks/results/extraction/2026-09-16/SPEECH-ACT-V12.md) passed 14/14 cases and all 15 safety/utility targets with zero unsafe claims. | Authored local probes and one hosted model profile, not held-out accuracy. V8–v11 failures are retained and show temperature-zero output variation. V12's narrow exact-source/entity recovery has causal local tests, but the live passing run did not activate it. The validator is not general entailment and does not cover every speech act. |
