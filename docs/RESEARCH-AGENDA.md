...
| Atomic organizer merges | Relationship copies now retain validity/provenance and retry identities, and a copy failure keeps the source active. Next, publish canonical evidence, copied edges and supersedence in one backend transaction with concurrent-writer and crash tests. Partial copies and unrelated concurrent merges are not yet covered by an atomic publication protocol. |
