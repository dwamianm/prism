...
|---|---|
| Named projects and domains | `MemoryWorkspace` now manages named local projects with stable pack identity, bounded lease ownership, shared embeddings, eviction/reopen and copy checks. The [workspace guide](WORKSPACES.md) states cancellation, background-extraction and operator boundaries. Next: PostgreSQL partition/pool implementation and benchmarks; hosted credential-to-project grants; explicit legacy import and lifecycle operations. A metadata filter alone is insufficient, and the full grant hierarchy remains unimplemented. |
