...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Schema 3 localizes speech-act and negation guards to retained sentence-numbered evidence. Its [registered causal confirmation](../benchmarks/results/research/2026-09-16/WICE-CLAIM-VERIFICATION-LOCALIZED-V2.md) preserved all eight authored gates and improved WiCE recall from 10.91% to 30.91%, while still accepting 0/32 fully unsupported claims, but failed unchanged precision, recall and balanced-accuracy gates. A pinned MIT-licensed [MiniCheck DeBERTa capacity trial](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-DEBERTA-V1.md) then improved F1 to 65.97% and balanced accuracy to 75.01% with 0/32 fully unsupported accepts. It still failed 90% precision and 60% recall gates at 77.78% and 57.27%; no threshold over frozen scores can satisfy both. | WiCE uses perfect evidence retrieval, so these are not PRME retrieval or original full-document results. The cohort was observed before model selection, though MiniCheck's weights and published threshold predate this run. Keep verification opt-in. Test the published MIT Flan-T5 variant without tuning, then require another external cohort and model family before promotion. |
