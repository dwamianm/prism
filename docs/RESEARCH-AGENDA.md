...
Updated 2026-09-16. This agenda supersedes the [v0.6 proposal](archive/RESEARCH-AGENDA-v0.6.md).
...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Schema 3 localizes speech-act and negation guards to retained sentence-numbered evidence; its [registered causal confirmation](../benchmarks/results/research/2026-09-16/WICE-CLAIM-VERIFICATION-LOCALIZED-V2.md) preserved all eight authored gates but still failed external precision, recall and balanced-accuracy gates. Pinned MIT MiniCheck capacity trials improved the model ceiling: [DeBERTa](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-DEBERTA-V1.md) reached 65.97% F1 and [Flan-T5](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-FLAN-V1.md) reached 74.77% F1. A preregistered [atomic follow-up](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-FLAN-ATOMIC-V1.md) showed the human all-subclaim rule reconstructs 357/358 parents, but unchanged MiniCheck reached only 76.04% precision and 66.36% recall at that boundary. All trials accepted 0/32 fully unsupported parents; no frozen-score threshold meets both precision and recall gates. | WiCE uses perfect evidence retrieval, so these are not PRME retrieval or original full-document results. The cohort was observed before model selection, though weights and published inference rules predate PRME's runs. Keep verification opt-in. A product decomposition API now needs a materially different atomic verifier and evidence on another bound external cohort before promotion. |
