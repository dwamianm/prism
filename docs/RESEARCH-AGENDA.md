...
| Local claim verification | The registered [20-case minimal-evidence assay](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-MINIMAL-GROUPS-V1.md) found both unseen two-passage chains, every explicit refutation and conflict, and both completeness boundaries, but overcalled two neutral passages as refutations. The deterministic guard developed from that failure then passed all eight gates in a [same-cohort confirmation](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-GUARDED-REFUTATION-V2.md): 19/20 expected statuses, zero unsafe supports and zero false refutations among expected-insufficient cases, with raw model scores preserved. | Authored development probes with one pinned DeBERTa NLI model. The confirmation cohort was fully observed and the guard was designed against its failures. One support paraphrase remains missed. Broader untouched claims must measure implicit-contradiction recall and the precision/recall cost before refutation enforcement or a quality claim. |
