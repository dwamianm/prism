...
| Adaptive retrieval and memory credit | Immutable owner-scoped receipts, explicit relevance labels and answer-time citations now preserve returned, packed and cited memory identities across restart. Citation sets accept an explicit no-citation answer and reject hidden/reference-only entries. Next: reproduce [Hindsight Memory-PRM](https://arxiv.org/abs/2608.29605) deletion-and-reanswer credit on fixed readers and held-out tasks, then complete scoped profile activation and rollback. Citations alone are not answer correctness, negative labels, or causal value. Global weights and retention must not silently consume one tenant's observations. See RFC-0017. |
