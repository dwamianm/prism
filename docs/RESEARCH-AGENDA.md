...
| Storage and developer workflow | Regression at `ab9e3b9`: 2,663 passed, 81 skipped across main and research/example invocations, including live PostgreSQL. Installed Python 3.13 provider/recovery checks passed 154 tests; the real-BGE sync/async workflow preserved source data and encoding identity. Malformed embedding batches cannot partially populate the cache or replace existing vectors. | Tests establish their covered contracts, not answer quality or every deployment environment. |
