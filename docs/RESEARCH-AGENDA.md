...
| Exact quantity extraction | Fresh `speech_act_v10` output preserves source-verbatim decimal strings, bounded units, polarity, conditions and target-bearing objects. The registered [DeepSeek development pass](../benchmarks/results/research/2026-09-18/QUANTITY-EXTRACTION-DEEPSEEK-DEV-V5.md) and [local Qwen 35B-A3B confirmation](../benchmarks/results/research/2026-09-18/QUANTITY-EXTRACTION-QWEN-CONFIRM-V3.md) passed 40/40 combined cases and 24/24 expected quantity instances with zero extras or provider failures. Earlier failed trials remain preserved and causally localize float transport, clipped approximations, omitted dimensionless claims, provider-dependent unit fields, dropped conditional claims and duplicate fallback. | Authored cases through one Ollama-compatible interface, not held-out extraction accuracy or exhaustive unit coverage. Source-derived repair uses a bounded lexicon and never converts units. Exact aggregation still reports extraction and real-world coverage limits; natural-language query planning and end-to-end aggregation answers remain separate gaps. |
