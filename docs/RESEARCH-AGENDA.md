...
| Jev decision support | A pinned TypeSafe Jev product protocol improved held-out proposal recall from 2.50% to 78.00% and accuracy from 51.00% to 87.50% versus normalized exact names, at 96.30% precision. PRME exposes it as an opt-in advisor and can atomically publish positive results as audited, unverified alias proposals. Earlier held-out beer alignment similarly improved recall from 14.29% to 64.29%, but its 94.74% precision missed the 98% automatic-merge gate. | Jev is validated for proposal advice on the tested product records, not silent identity mutation or arbitrary domains. A fresh retrieval reranker regressed hit@1 by 9 points, and general claim verification missed its precision gate; both uses are rejected. Next validate end-to-end product candidate generation and add a first-class proposal review/decision workflow before expanding runtime use. |
