...
| Developer experience | Installed-wheel sync/async workflows cover confirmation retries and [explicit corrections](MEMORY-CORRECTIONS.md), including retained sources and scoped evidence. [Metadata admission](METADATA.md) now rejects nonportable values consistently and preserves legacy values in journals. Keep testing first write, restart, retrieval, recovery and migration without repository-only imports; these checks do not establish end-to-end memory quality. |
