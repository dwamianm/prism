...
| Evidence-bound temporal relations | The frozen disjoint [104-question confirmation](../benchmarks/results/research/2026-09-18/LONGMEMEVAL-S-TEMPORAL-RELATION-CONFIRMATION-V2.md) improved answer accuracy from 50/104 to 60/104, with 10 paired wins, zero losses and all 5 abstention controls preserved. The accepted subset improved from 11/22 to 21/22. The current [product-path replay](../benchmarks/results/research/2026-09-18/LONGMEMEVAL-S-TEMPORAL-RELATION-PRODUCT-PARITY-V3.md) matched all 104 frozen contexts and receipts, reproduced 22/22 accepted changes, and reduced resolver routing from 100 to 82 questions without missing a changed case. A [500-pack regression](../benchmarks/results/research/2026-09-18/LONGMEMEVAL-S-TEMPORAL-RELATION-FULL-REGRESSION-V1.md) then matched every context and receipt, reproduced all 31 frozen development/confirmation changes, and reduced the full call surface from 195 to 145. | One held-out partition of the same LongMemEval-S source, one hosted resolver/reader alias, a custom judge and an external gate. The replays used frozen provider outputs; 38 routed non-temporal-category questions used an inert resolver. Payload sizes are measured, while live availability, billed tokens and latency remain unmeasured. This supports the exact composition, not universal accuracy or a competitor claim. Keep it disabled by default pending a bounded live-provider preflight. |
...
| Temporal and aggregate questions | Present-state and dated-history routing pass the causal simulation gate. The opt-in evidence-bound temporal relation path passed its disjoint answer confirmation, reproduced all 104 confirmation contexts through the product API, and passed a 500-pack routing/context/receipt regression. Semantic counts/lists expose non-exhaustive coverage, every observed cap/failure/selection limit, and a token-counted model warning; stored-record enumeration remains explicit. Next: run the bounded live-provider preflight, then evaluate semantic qualification, real-item deduplication, multi-session updates and abstention together. |
