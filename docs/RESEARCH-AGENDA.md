...
| Hierarchical summary maintenance | Daily, weekly and monthly source excerpts now use durable prepared publications with deterministic period lineages. Unchanged, concurrent and restarted runs converge on one active identity; changed selected inputs and legacy excerpts are replaced atomically. | Authored consistency and recovery tests. This does not establish that extractive summaries improve answer quality or provide exhaustive coverage. |
