...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Causal guards and caller-order preservation passed same-cohort confirmations. The [44-case authored guard confirmation](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-GUARD-ALIGNMENT-V2.md) passed all eight targeted gates at 43/44. The first [external WiCE oracle-retrieval assay](../benchmarks/results/research/2026-09-16/WICE-CLAIM-VERIFICATION-V1.md) then evaluated all 358 human-annotated test claims. It accepted 0/32 fully unsupported claims and had a 0.81% overall false-support rate, but failed precision, recall and balanced-accuracy gates: passage-wide modality and negation guards reduced supported recall to 10.91%, versus 41.82% for identical raw scores. | The external result is verification after WiCE's perfect evidence retrieval, not PRME retrieval or original full-document performance. It exposes a passage-scoping defect and blocks default promotion. The pinned base NLI model also misses most supported claims at the registered threshold. Localize guards without losing completed-action safety, then require a new untouched external cohort and another model family. |
