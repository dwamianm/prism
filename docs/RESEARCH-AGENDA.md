...
| Local claim verification | The registered [20-case minimal-evidence assay](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-MINIMAL-GROUPS-V1.md) exposed unsafe model-only refutation, leading to a guarded same-cohort pass. A later [48-case previously unexecuted assay](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-BROAD-DEV-V1.md) matched 40/48 but failed with two unsafe supports. Speech-act support gating and a separately identified near-exact negation basis then passed a [same-cohort safety confirmation](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-BROAD-SAFETY-V2.md). Preserving caller order after bounded candidate selection subsequently passed all eight gates in an [order confirmation](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-BROAD-ORDER-V3.md): 44/48, zero unsafe supports, 18/18 support cases, 6/6 minimal groups, 12/12 insufficient cases, 4/4 conflicts and 4/4 completeness refusals. | All passing confirmations reused observed failure cohorts; the order diagnostic directly preceded v3. Four implicit contradictions remain `insufficient` without explicit correction or typed exclusivity. The model is order-sensitive, and preserving caller order does not establish order invariance. These are authored probes with one pinned model. Keep verification opt-in until an untouched cohort tests false-positive risk, typed modality and different evidence orders. |
