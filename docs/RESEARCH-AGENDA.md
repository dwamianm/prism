...
| Local claim verification | The registered [20-case minimal-evidence assay](../benchmarks/results/research/2026-09-16/CLAIM-VERIFICATION-MINIMAL-GROUPS-V1.md) matched 17 cases with zero unsafe supports. Both unseen two-passage chains were neutral as singles and supported as bounded pairs; all explicit refutations and conflicts were found, and exhaustive claims returned `incomplete` without a model call. | Authored development probes with one pinned DeBERTa NLI model. The all-cases and support gates failed. Two neutral passages were incorrectly called refutations, so contradiction probability alone is not a production refutation policy. |
