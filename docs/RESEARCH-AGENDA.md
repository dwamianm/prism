...
| Local claim verification | Early registered assays exposed unsafe model-only refutation, speech-act collapse, missed conflicts and evidence-order sensitivity; each failed result is retained. Schema 3 localizes guards to retained evidence but still fails external gates. Pinned MIT MiniCheck trials culminated in a preregistered [atomic follow-up](../benchmarks/results/research/2026-09-16/WICE-MINICHECK-FLAN-ATOMIC-V1.md): human decomposition reconstructed 357/358 parents, while the model reached 76.04% precision and 66.36% recall. A fresh ten-domain [SummEdits proof trial](../benchmarks/results/research/2026-09-16/SUMMEDITS-PROOF-VERIFICATION-V1.md) then rejected free-form model quotations: 75.00% precision, 6.00% recall, 52.00% balanced accuracy and 23.00% quote integrity. | WiCE assumes perfect retrieval and SummEdits uses controlled summary edits with repeated source families. Keep verification opt-in. The next development protocol separates summary-only decomposition from fixed-atom verification and uses deterministic segment IDs instead of copied quotations; require an untouched confirmation before product promotion. |
