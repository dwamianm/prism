...
| Exact quantity extraction and aggregation | Fresh `speech_act_v10` output preserves source-verbatim decimal strings, bounded units, polarity, conditions and target-bearing objects. The registered [DeepSeek development pass](../benchmarks/results/research/2026-09-18/QUANTITY-EXTRACTION-DEEPSEEK-DEV-V5.md) and [local Qwen 35B-A3B confirmation](../benchmarks/results/research/2026-09-18/QUANTITY-EXTRACTION-QWEN-CONFIRM-V3.md) passed 40/40 combined cases and 24/24 expected quantity instances with zero extras or provider failures. The first [durable end-to-end aggregation assay](../benchmarks/results/research/2026-09-18/QUANTITY-AGGREGATION-E2E-QWEN-V1.md) then completed ten natural-language sources with no provider failure and safely excluded unrelated, negated, conditional, approximate, incompatible-unit and cross-owner values. | The end-to-end product gate failed: an open-vocabulary predicate suffix excluded one correctly stored `$250` contribution, producing `$3,500` instead of `$3,750`, and the same complex source dropped an independent `5 kilometers` claim. Authored cases through one Ollama-compatible interface do not establish held-out extraction accuracy or exhaustive unit coverage. Source-derived repair uses a bounded lexicon and never converts units. Predicate composition and natural-language query planning remain separate gaps. |
