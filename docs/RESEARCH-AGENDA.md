...
| Developer experience | Installed-wheel sync/async workflows cover confirmation retries and [explicit corrections](MEMORY-CORRECTIONS.md), including retained sources, scoped evidence, full audit records, HTTP/MCP parity and exact retry safety. [Metadata admission](METADATA.md) rejects nonportable values and ambiguous JSON key collisions consistently, and preserves legacy values in journals. Keep testing first write, restart, retrieval, recovery and migration without repository-only imports; these checks do not establish end-to-end memory quality. |
