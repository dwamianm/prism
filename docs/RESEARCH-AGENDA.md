...
| Interactive task completion | The [MemoryArena adapter](../benchmarks/results/research/2026-09-12/AGENTIC-MEMORY-INTEGRATION.md) passed the unmodified upstream client with installed PRME, real embeddings, task isolation and restart provenance. The corrected travel trial then completed a fresh 12-group [confirmation](../benchmarks/results/research/2026-09-17/MEMORYARENA-TRAVEL-DEEPSEEK-CONFIRM12-V1.md): PRME used 19.76% fewer input tokens but scored 0.00 strict PS and 65.12 SPS versus native history at 7.69 and 80.24, failing both -5-point non-inferiority gates. Post-hoc localization found one exact-value rendering failure contributed 40 of the net 56 lost slots even though the full base plan was retrieved. Prompt-only V12/V13 repairs passed broad development gates but failed targeted inspection. [V14 typed bindings](../benchmarks/results/research/2026-09-17/MEMORYARENA-TRAVEL-DEEPSEEK-DEV12-V14.md) improved exact qualified values from V11's 239/318 to 244/318 and eliminated qualified tool calls, but missed the strict PS floor by 0.1282 points. [V15 tool-boundary resolution](../benchmarks/results/research/2026-09-18/MEMORYARENA-TRAVEL-DEEPSEEK-DEV12-V15.md) matched all 620 executions, applied 88 exact source-backed replacements and sent zero qualified arguments to tools; it passed both broad gates at +11.54 PS and -3.93 SPS while using 25.15% fewer input tokens, but final-answer fidelity fell to 234/318 and the model requested 84 qualified calls. | The opt-in typed presentation/lookup API has direct workflow evidence for safe execution-boundary resolution, but no MemoryArena integration has passed both the broad and targeted selection rules. Keep confirmation sealed. Next attach canonical presentation metadata only to results of calls that used an exact binding; require zero executed qualified arguments, at least 244/318 exact qualified outputs, and both broad gates before confirmation. The mutable hosted alias, one generation per arm and exact-string scorer prevent broader claims. |
