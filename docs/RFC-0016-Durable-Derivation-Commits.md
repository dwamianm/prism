...
`grounding_policy="speech_act_v9"`, and plans made from those records use
...
same exactness, evidence, condition and reference validation. Saved v8, v7 and
...
speech act in their predicate; they and relationships between components inside
...
When the model omits that relation, recovery is limited to a literal nonactual
clause and the first exact model-returned entity after its action verb. Explicit
if/unless conditions remain conditional. It does not search memory, resolve
pronouns, or infer an unreturned entity.
...
`grounded_quantities_v6`, `temporal_validity_v7`, `speech_act_v8`, `speech_act_v9`, `speech_act_v10`, and `speech_act_v11` plans remain readable and
