...
Historical plans through materialization policy `temporal_validity_v7` keep
their original behavior. Fresh built-in inference records
`grounding_policy="speech_act_v2"`, and plans made from those records use
`speech_act_v8`: relationship
...
speech act in their predicate; they cannot become current-state relations.
...
Extraction records whose omitted policy defaults to `source_passage_v1` prepare
...
`grounded_quantities_v6`, and `temporal_validity_v7` plans remain readable and
