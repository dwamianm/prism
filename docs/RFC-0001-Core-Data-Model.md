...
| `PARTICIPANT` | Content produced by another human in the conversation, such as the second person in a two-person chat. |
...
`USER` and `PARTICIPANT` content is first-party (`USER_STATED`, Section 7);
`ASSISTANT` and `SYSTEM` content is `SYSTEM_INFERRED`. A `PARTICIPANT` is not the
memory's owner, so a participant's repeated statement does not reinforce an
instruction (RFC-0003, Section 9). An event can also carry an optional speaker
name, kept in the reserved `prme_speaker_v1` metadata key, that says which person
spoke. It is an unverified caller assertion and does not change the role.
...
|---|---|
| `USER_STATED` | A human in the conversation directly stated this: the user, or another participant. |
