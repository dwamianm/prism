...
Historical `speech_act_v10` and earlier plans keep their exact node
metadata and replay unchanged. Fresh built-in extractions record
`speech_act_v5`, and plans made from those outputs record `speech_act_v11`, which
...
relationships inside the attempted action. If only an attempt count or failure
detail survives, v11 can recover the first exact model-returned entity after the
literal tried/attempted action verb. Saved v4 records prepare v10, v3 prepare v9,
and v2 prepare v8. A legacy extraction
