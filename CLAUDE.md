...
- Default changes have the owner's standing approval (2026-09-23) when a DeepSeek paired answer run at the 4K budget on both LoCoMo and LongMemEval-S, over the same questions, shows (a) a gain in overall accuracy on at least one benchmark whose paired 95% interval excludes zero, and (b) no loss on the other benchmark whose paired 95% interval excludes zero. Repeated DeepSeek runs on identical contexts do not give identical answers, so the rule also needs these (#118):
  - **Confirmation run.** A variant that passes must pass the same test a second time, with a fresh answer run of the variant against the same baseline, before the default flips. This guards against a lucky draw of the variant and against the extra false passes that come from testing many variants against one baseline; it does not draw the baseline again. The confirmation is the first fresh run with the same settings: if it fails, the variant fails. The pull request lists every arm prepared with those settings and every baseline it was paired with (#130).
  - **Model identity.** Every comparison uses the same Ollama model identity and answer settings as its baseline, and `compare` refuses a mismatch. If either changes, record a new baseline first. The identity is Ollama's local manifest digest and the hosted weights are not pinned, so a matching identity does not prove the same weights.
  - **Run-to-run floor.** The paired difference between two answer runs of the defaults with the same inputs (`compare` reports such a pair as a repeat) is recorded in `BENCHMARKS.md` for the model identity it was measured with; measure it again when the identity changes. The floor holds when its 95% interval includes zero on both benchmarks. If it excludes zero on either one, the paired test is not trustworthy as it stands, and no default changes until the test is revised.
  - **Current state (2026-09-24).** The floor does not hold: a repeat of the defaults gained 2.2 points on LongMemEval-S with a 95% interval of +0.4 to +4.2 points. No default changes until the paired test is revised (#129).

  The pull request that flips a default must include the before and after numbers of both runs, the intervals and the receipt paths. After a flip, record a new DeepSeek baseline for the new defaults and compare later changes against it.
