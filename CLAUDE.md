...
- Default changes have the owner's standing approval (2026-09-23) when a DeepSeek paired answer run at the 4K budget on both LoCoMo and LongMemEval-S, over the same questions, shows (a) a gain in overall accuracy on at least one benchmark whose paired 95% interval excludes zero, and (b) no loss on the other benchmark whose paired 95% interval excludes zero. The pull request that flips a default must include the before and after numbers, the intervals and the receipt paths. The first flip waits until the run-to-run noise floor is measured (#118), because repeated DeepSeek runs on identical contexts do not give identical answers. After a flip, record a new DeepSeek baseline for the new defaults and compare later changes against it.
