...
- **Append-only**: Events must never be overwritten or deleted except by policy-based archival. Conflicting assertions must not silently overwrite prior ones; use supersedence. Note: store-time supersedence is gated behind `enable_store_supersedence` (default `False`, `src/prme/config.py`); with the default, `store()` does not supersede and conflict handling relies on retrieval-time recency (under the default rank fusion, the current-state recency boost and event-time tie-break) and chronological `[MOST RECENT]`/`[OLDER]` markers in the context formatter rather than graph-level supersedence edges; recency alone is not treated as proof of truth. Predicate matching in the supersedence detector is exact-match plus three hardcoded equivalence classes (`src/prme/ingestion/supersedence.py`); paraphrased predicates are not currently superseded.
...
Config is defined as Pydantic models in `src/prme/config.py` and `src/prme/retrieval/config.py` (loaded from `PRME_`-prefixed env vars, `.env`, or direct args). The surface is large (roughly 100 fields across both files). Several parameter defaults are explicitly tagged `[HYPOTHESIS]` in their descriptions; these are reasoned but not yet benchmark-validated and may change. Treat `[HYPOTHESIS]` knobs as provisional and prefer not to depend on their exact values. Notable defaults to be aware of: the retrieval defaults changed after v0.12.0 (#177, #187) to rank fusion (`scoring.fusion="rrf"`, `rrf_k=60`) with `scoring.rrf_recency_boost=0.25` and `scoring.rrf_tie_break="event_time"`, the `reader` context format, `balanced` multi-path ordering and `packing.session_context_rank_fusion_score_decay=0.6`. They live in `PRMEConfig`; `ScoringWeights()` and `PackingConfig()` built in code keep their field defaults (weighted; `auditable`, `balanced` and no rank fusion session decay). `docs/PACKING.md` ("Default retrieval settings") lists the environment variables that restore the v0.12.0 defaults. Also `enable_store_supersedence=False`, `enable_surprise_gating=False`, `enable_qa_pairing=False`, and `enable_reranker=False` (on the offline evidence gate, no cross-encoder rank-order variant beat rank fusion alone on both benchmarks at both 4K and 8K, so the reranker stays off; #88).
