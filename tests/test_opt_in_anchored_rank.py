...
from benchmarks.diagnostics.opt_in_anchored_rank import AnchoredRankEnvelopeReranker, original_anchor
...
async def test_authored_source_executor_replays_prior_and_rejects_drift(tmp_path, monkeypatch):
    import json
    ...
    result = await current.evaluate(case, {'configuration': arm['config']}, earlier, tmp_path / 'current')
    ...
    assert all(v['evidence']['complete_source_literal_recall'] for v in record['arms'].values())
    ...
    with pytest.raises(current.study.old.ResearchFailure, match='replay differs'):
        await current.evaluate(case, {'configuration': arm['config']},
            {**earlier, 'case_sha256': file_sha(path)}, tmp_path / 'bad')
