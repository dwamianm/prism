...
def test_snapshot_owner_or_scope_collision_fails_atomically(field, value):
    ...
    with pytest.raises(ValueError, match='source snapshots'):
        policy.merge_signals([base], [[candidate(2, ['VECTOR'], .9), other]])
...
async def test_alternate_backend_failure_does_not_commit_partial_merge(monkeypatch):
    ...
    engine = SimpleNamespace(_query_reformulation_provider='ollama', _query_reformulation_model='authored',
        ...
        _vector_index=None, _lexical_index=None, _research_merge_trace=[])
    values = [candidate(1, ['VECTOR'], .3)]
    with pytest.raises(RuntimeError, match='backend failed'):
        await policy.expand_with_merge(engine, 'telescope', candidates=values, user_id='authored',
...
async def test_complete_authored_assay_and_frozen_input_rejection(tmp_path, monkeypatch):
    from benchmarks.diagnostics import opt_in_reformulation_merge_study as assay
    from benchmarks.diagnostics.register_opt_in_interactions import clean_config, file_sha
    ...
    monkeypatch.setattr('prme.retrieval.reformulation.reformulate_query', assay.cached_reformulation)
    record = {'query': case['question'], 'alternatives': ['blue telescope', 'color preference'],
        'options': {'provider': reform['extraction']['provider'], 'model': reform['extraction']['model'],
                    'count': reform['query_reformulation_count']}, 'calls': 0}
    frozen = {}
    for arm, configuration in [('baseline', config), ('query_reformulation', reform)]:
        ...
        try:
            with assay.study.matched_admission():
                await assay.study.capture(case, {'config': configuration, 'stratum': 'fresh'}, folder, None)
        finally: assay.CACHED.reset(token)
        (folder / 'result.json').write_text('{"status":"complete"}')
        (folder / 'feature-observations.json').write_text(json.dumps({'reformulations': [
            {'query': record['query'], 'alternatives': record['alternatives']}] * 2}))
        frozen[arm] = {'authored': {'capture_sha256': file_sha(folder/'capture.json'),
            'feature_observations_sha256': file_sha(folder/'feature-observations.json')}}
    reg = {'configuration': config, 'reformulation_configuration': reform, 'input_cases': frozen}
    ...
    assert all(v['evidence']['complete_source_literal_recall'] and v['context_tokens'] <= 3996
               for v in result['arms'].values())
    assert result['new_hosted_model_calls'] == 0 and result['local_query_embeddings_reexecuted']
