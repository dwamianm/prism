...
Fresh built-in extractions record `grounding_policy="speech_act_v4"`; plans
made from those records use `speech_act_v10`. Version 10 also rejects completed
component relationships inside a first-person attempt clause and requires the
attempted target even when the extractor also records a count or failure. Saved
v3 records recover under v9, v2 records recover under v8; older extraction records
default to `source_passage_v1` and remain on `temporal_validity_v7` when recovery
must prepare a missing plan. Version 10 preserves the
