...
that separate multiplier was disabled and omit it. Version 10 records the
...
deterministic retrieval pipeline. Provider failures are not verdicts, and
assessments do not mutate memory or retrieval receipts. A registered repeated
...
and the envelope policy keeps its fused scores in that order (issue #88). A
weight other than 0.3 requires an envelope policy (refused with `"legacy"`) and
is recorded in receipts and the reranker's feature identity; 0.3 is omitted, so
earlier receipts keep their bytes.
