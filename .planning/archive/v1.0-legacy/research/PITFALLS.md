...
2. **Use ** — DuckDB added SQL/PGQ graph syntax support, allowing graph queries within the same DuckDB instance used for the event store. This eliminates a separate dependency entirely. Risk: DuckPGQ is a community extension, not core DuckDB.
...
_Pitfalls research for: Local-first LLM memory engine with event sourcing, graph modeling, hybrid retrieval, and self-organization_
